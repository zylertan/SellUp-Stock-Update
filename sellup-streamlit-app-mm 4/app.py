...
if master_file and sellup_file:
    ...
    # ---- carry-forward safeguard (prevents silent loss of past matches) ----
    if registry_file is not None:
        if crosswalk:
            st.success(f"🔒 Carried forward {len(crosswalk)} confirmed matches "
                       "from your uploaded registry.")
        else:
            st.error("⚠ No previous matches were read from your registry. Make sure "
                     "you're uploading your completed **SellUp_Match_Review.xlsx** "
                     "(the one with the Locked Matches tab) in box 3 — otherwise "
                     "previously-linked SKUs will reappear as new.")
    else:
        st.warning("⚠ No reviewed registry uploaded in box 3 — previously-confirmed "
                   "matches will NOT be carried forward and will reappear for review. "
                   "Upload your last SellUp_Match_Review.xlsx to keep them locked.")
