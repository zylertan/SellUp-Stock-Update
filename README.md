...
## Files (put these three in a GitHub repo)
- `app.py` — the Streamlit interface
- `sellup_core.py` — all the matching logic
- `requirements.txt` — dependencies

## How it runs (same flow as Shopee)
1. **Upload** the Masterlist (`stock_report*.xlsx`), the SellUp export
   (`INVENTORIES_*.xlsx`), and the SellUp Data crosswalk. Optionally re-upload a
   previously reviewed registry.
